...
**考虑因素**：
- 传统一对一部署模式（一个Agent一个Pod）导致资源浪费
- 空闲Agent占用Pod资源但无实际工作
- 任务突发时资源不足，需要弹性扩容
...
spec:
  ...
  harness:
    model:
      - name: model-openai-default   # 模型服务配置
    mcp:
...
模型作为平台提供的核心服务能力，通过Harness配置Agent可以访问的模型服务。
...
**考虑因素**：
- 模型服务是Agent的核心依赖，需要统一配置和管理
- 平台层提供模型接入能力，Handler无需处理不同提供商的接入细节
- 支持多模型配置，Agent可根据任务需求选择合适模型
- 模型访问控制和限流策略，避免资源滥用
- 认证信息通过Secret管理，符合K8s安全实践

#### 5.3.2 MCP类型特殊设计

由于MCP Server数量庞大且种类繁多，无法在Harness中枚举所有具体的MCP Server。因此MCP Harness采用**Registry模式**，只配置MCP Registry的连接信息以及允许发现和禁止发现的MCP Server策略。
