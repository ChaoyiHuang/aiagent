	...
	*notFoundCount = 0
	...
	for _, entry := range index.Agents {
		...
		if processMode == handler.ProcessModeIsolated {
			...
			// IMPORTANT: Do NOT call StartAgent after this - it would cause deadlock
			// because StartAgent internally calls StartFrameworkInstance which locks again
			...
			// Update agent phase directly (avoid deadlock from StartAgent double-lock)
			if info := h.GetAgent(entry.Name); info != nil {
		...
		if processMode == handler.ProcessModeShared {
			...
			// Shared mode: use StartAgent to register agent (no deadlock - it doesn't call StartFrameworkInstance)
			if ag != nil {
